...
export async function initGenerator(tree: Tree, options: InitGeneratorSchema) {
  ...
  if (existingConfig.hasNxJsonConfig || existingConfig.hasProjectConfigs) {
    ...
    if (existingConfig.hasProjectConfigs) {
      logger.warn(
        `   - ${existingConfig.configuredProjects.length} projects have release targets`
      );
      logger.warn(
        `     Projects: ${existingConfig.configuredProjects.join(', ')}`
      );
    }
    logger.info('');
  ...
  if (!options.skipPrompts) {
    ...
    logger.info(
      `CI-Only Releases:    ${
        answers.ciOnly ? 'Yes (recommended)' : 'No (allows local releases)'
      }`
    );
    ...
    if (
      answers.configureReleaseGroups &&
      answers.releaseGroupsConfig &&
      answers.releaseGroupsConfig.length > 0
    ) {
      logger.info('');
      logger.info(
        `Release Groups:      ${answers.releaseGroupsConfig.length} configured`
      );
      for (const group of answers.releaseGroupsConfig) {
        logger.info(
          `  - ${group.groupName}: ${group.versioning} (${group.projectsPattern})`
        );
      }
    }
    ...
    if (
      answers.configLocation === 'nx-json' ||
      answers.configLocation === 'both'
    ) {
    ...
    if (
      answers.configLocation === 'project-json' ||
      answers.configLocation === 'both'
    ) {
    ...
    logger.info(
      `    nx run-many -t ${
        answers.executorType === 'individual' ? 'version' : 'project-release'
      }`
    );
    logger.info('');
  } else {
    ...
    logger.info(
      'Running in non-interactive mode with default configuration...'
    );
    logger.info('');
    ...
    if (publishableProjects.length === 0) {
      logger.warn(
        '⚠️  No publishable projects found (projects with package.json)'
      );
  ...
  return () => {
    ...
    if (commitizenEnabled) {
      logger.info(
        '💡 Tip: Use `npm run commit` for guided conventional commits'
      );
      logger.info('');
    }
