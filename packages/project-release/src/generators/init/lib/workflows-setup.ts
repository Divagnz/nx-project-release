...
export function createGitHubWorkflows(
  tree: Tree,
  workflowType: 'single' | 'two-step' | 'affected',
  projectNames: string[]
): void {
  if (workflowType === 'affected') {
    createAffectedWorkflows(tree, projectNames);
...
function createAffectedWorkflows(tree: Tree, projectNames: string[]): void {
  ...
  create-batch-release-pr:
    ...
    steps:
      ...
      - name: Version affected projects
        id: version
        run: |
          # Version all affected projects in one commit
          ...
          # Collect bumped projects and versions
      ...
      - name: Create Pull Request
        run: |
          gh pr create \\
            --title "chore(release): release \$(date +%Y-%m-%d)" \\
            --body "## 📦 Release\\n\\nVersion bump: **\${{ inputs.releaseType }}**\\n\\n### Bumped Projects\\n\\n\${{ steps.version.outputs.bumped_projects }}\\n---\\n*Generated by nx-project-release*" \\
            --base \${{ inputs.baseBranch }} \\
